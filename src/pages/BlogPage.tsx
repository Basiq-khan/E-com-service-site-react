    ...
    <div className="pt-20">
      ...
      <section className="bg-[#75221D] min-h-[60vh] md:h-[70vh] flex items-center relative overflow-hidden py-20 md:py-0">
        <motion.div 
          ...
          className="absolute -top-40 -left-40 w-[400px] md:w-[800px] h-[400px] md:h-[800px] bg-[#FE502D]/10 rounded-full blur-[120px]"
        />
        <div className="container mx-auto px-4 relative z-10 text-center uppercase italic">
           <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="inline-block px-6 py-2 bg-white/10 backdrop-blur-md border border-white/10 rounded-[5px] text-[#FDF392] text-[10px] md:text-xs font-black tracking-[0.5em] mb-6 md:mb-8"
           >
      ...
      <section className="py-20 md:py-32 bg-white relative overflow-hidden">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 md:gap-16 max-w-7xl mx-auto px-4 md:px-0">
            {posts.map((post, i) => (
              <motion.div 
                key={i}
              ...
              >
                <div className="relative overflow-hidden rounded-[5px] mb-6 md:mb-8 bg-gray-50 border border-gray-100 aspect-[16/10]">
                   <img src={post.image} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                   <div className="absolute top-4 left-4 md:top-6 md:left-6 bg-[#FE502D] text-white text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-[5px] shadow-2xl skew-x-[-10deg]">
                      {post.category}
                   </div>
                </div>
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                     ...
                     <p className="text-[#FE502D] font-black text-[10px] md:text-xs uppercase tracking-[0.2em]">{post.date}</p>
                  </div>
                  <h3 className="text-2xl md:text-3xl font-black text-[#75221D] uppercase italic tracking-tighter leading-[1.1] group-hover:text-[#FE502D] transition-colors line-clamp-2 md:line-clamp-none">{post.title}</h3>
                  <p className="text-gray-500 text-sm md:text-base leading-relaxed line-clamp-2">{post.excerpt}</p>
                  <div className="pt-2 md:pt-4 flex items-center gap-3 text-[#75221D] font-black uppercase italic tracking-tighter text-lg group-hover:gap-6 transition-all duration-300">
